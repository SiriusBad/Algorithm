...
def RMSProp(x_init, learning_rate, iterations, decay):
    ...
    for i in range(iterations):
        ...
        f_history.append(f(x))
        gradient_x=gradient(x)
        G_x = decay * G_x + (1 - decay) * (gradient_x ** 2)
        x -= (learning_rate / (np.sqrt(G_x) + epsilon)) * gradient_x
    plot_curve('RMSProp',x_history, f_history)

def Momentum(x_init, learning_rate, iterations, decay):
    x_history = []
    f_history = []
    x = x_init
    G_x=0
    for i in range(iterations):
        ...
        f_history.append(f(x))
        gradient_x=gradient(x)
        G_x = decay * G_x - learning_rate * gradient_x
        x += G_x
    plot_curve('Momentum',x_history, f_history)
...
# Momentum(x_init, learning_rate, iterations, decay)
